...
NUM_PATH = os.path.join(config["data_path"]["processed_path"], "analisis_estadistico_numerico.csv")

PROMPT = """
<contexto>
{context}

Tu función es analizar los resultados de estadística descriptiva para las variables numéricas de una base de datos clínica. 
El análisis recibido está estructurado en formato CSV, donde cada fila corresponde a una variable, e incluye: 
número de observaciones (n), media, desviación estándar, mediana, rango intercuartílico (RIC) y valores mínimos y máximos.
</contexto>
...
Mostrar la tabla con los resultados del analisis descriptivo.
...
class NummericSummaryAgent(Agent):
    """"""
    def __init__(self):
        super().__init__()
        self.df = pd.read_csv(DF_PATH)
        self.df_num = pd.read_csv(NUM_PATH)
...
def generate_numeric_summary() -> str:
    """"""
    agent = NummericSummaryAgent()
