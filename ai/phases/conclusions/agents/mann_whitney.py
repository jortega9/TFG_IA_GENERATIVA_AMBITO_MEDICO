...
con el test de Mann Whitney U. El analasis recibido está estructurado en formato CSV donde cada fila
es una variable numerica analizada con la variable objetivo y cada columna es:
...
Titulo:
Analisis comparativo usando el test Mann Whitney U

Tabla del analisis del contenido:
Mostrar la tabla con los resultados del analisis comparativo.
Si la tabla es muy larga (no entra en una pagina), elimina aquellas variables que sean
menos significativas dejando algunas que a pesar de no ser significativas estaban cerca de serlo.
...
class MannWhitneySummaryAgent(Agent):
    """"""
    def __init__(self):
        super().__init__()
        self.df = pd.read_csv(DF_PATH)
        try:
            self.df_mann = pd.read_csv(MANN_PATH)
        except Exception:
            self.df_mann = None
...
def generate_mann_whitney() -> str:
    """"""
    agent = MannWhitneySummaryAgent()
