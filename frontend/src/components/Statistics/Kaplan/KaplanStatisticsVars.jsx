...
const KaplanStatisticsVars = ({csvKaplanPath, nameVar, plotK, plotS}) => {
    const [procesando, setProcesando] = useState(false);
    const [loading, setLoading] = useState(false);

    const [medianSurvivalTime, setMedianSurvivalTime] = useState([]);
    const [grupo, setGrupo] = useState([]);
    const [nPatients, setNPatients] = useState([]);
    ...
    const handleExecuteData = async () => {  
        ...
        try {
            ...
            //variable,group,median_survival_time,n_patients
            ...
            for (const variable in result.result) {
                ...
                nPatsArray.push({ variable, valor: n_patients });
                groupArray.push({ variable, valor: group });
            }
            setMedianSurvivalTime(msArray);
            setGrupo(groupArray);
            setNPatients(nPatsArray);
        ...
        <Box sx={{
            ...
            <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', height: '10%' }}>
                <Typography sx={{ color: '#4D7AFF', fontSize: '0.9rem' }}>
                    <strong> KAPLAN MEIER DE {nameVar.toUpperCase()} . </strong>
                </Typography>
                {/* <ThemeToggle /> */}
            </Box>
            ...
            {procesando ? (
                                            ...
                                            <Table size="small" sx={{ minWidth: 600, border: '1px solid #e0e0e0' }}>
                                                <TableHead>
                                                    <TableRow sx={{ backgroundColor: '#f1f5ff' }}>
                                                        ...
                                                        <TableCell sx={{ fontWeight: 'bold', border: '1px solid #ccc' }}>Grupo</TableCell>
                                                        <TableCell sx={{ fontWeight: 'bold', border: '1px solid #ccc' }}>Tiempo Medio de Supervivencia</TableCell>
                                                        <TableCell sx={{ fontWeight: 'bold', border: '1px solid #ccc' }}>Número Pacientes</TableCell>
                                                    </TableRow>
                                                </TableHead>
                                                <TableBody>
                                                    {medianSurvivalTime.map((item, idx) => {
                                                        ...
                                                        return (
                                                            ...
                                                            >
                                                                <TableCell sx={{ border: '1px solid #e0e0e0' }}>{item.variable.split('_')[0]}</TableCell>
                ...
                >
                    OBTENER KAPLAN MEIER DE {nameVar}
                </Button>
            )}
            </Box>
    );
